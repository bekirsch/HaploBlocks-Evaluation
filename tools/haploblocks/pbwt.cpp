...
int_all calc_haploblocks(string block_file, string out_path, int_all minimal_block_size, int_all buffer_size)
{
   ...
   out_file << '#' << nb_column << '\n';
    ...
    while (!file.is_end()){
        ...
        if (!file.is_end()){
            ...
            nb_blocks += add_block_file(ar_inter_max, size_ar_inter_max, P, out_file);
        }
        else{
