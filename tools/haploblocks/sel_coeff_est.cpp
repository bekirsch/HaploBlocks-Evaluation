...
const unsigned int EFF_POP_SIZE = 10000; // effective population size
...
double sHat(const int nr_chroms, const int block_size, const double r, const double delta_r)
{
	...
	const double y_0 = 1.0 / (2*EFF_POP_SIZE);
...
std::tuple<unsigned int, unsigned int> estimate_selection_coeff(
	...
	const std::string lookup_path, unsigned int chr_number)
{
	...
	unsigned int output_count = 0, filtered_count = 0, filter_t2_count = 0, filter_t1_count = 0, filter_both_count = 0;
	out_file << "witness,start,end,size,bp start,bp end,r frac,t [gen],sHat,freq\n";
	while (std::getline(in_file, line))
	{
		if (line.size() < 10)
		{
			if (line.size() > 0 && line.front() == '#') // header line containing chromosome number
			{
				line.erase(0,1);
				nr_chroms = std::stoul(line);
			}
			continue;
		}
		...
		double s_hat = sHat(nr_chroms, count, r, delta_r);
		
		
		bool filter_t2 {false}, filter_t1 {false};
		
		
		// filter blocks that are too old
		
		if (bp_start > 1e20 || bp_end < 1) continue;
		...
		double t_q = t_given_s_n_N(0.005, count, nr_chroms, 2*EFF_POP_SIZE) / (double) 2*EFF_POP_SIZE;
		double exp_q = -1+count/(1+count*t_q/2);
		double qsmin = std::pow(1-y,exp_q);
		double qsmax = std::max(0.0001, qsmin);
		double qmin = std::min(0.01, qsmax);
		
		
		t_threshold = t_freq(y, nr_chroms, qmin);
	...
	std::cerr << "\r" << output_count << " selection coefficient estimates written, "
		<< filter_t2_count << " filtered by coalescence filter, " << filter_t1_count << " filtered by allele age filter, " << filter_both_count << " filtered by both."<< std::endl;
	out_file.close();
	in_file.close();
	return std::make_tuple(output_count, filtered_count);
}
